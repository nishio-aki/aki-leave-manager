...
* **直面した壁:** 当初は失効期限別にデータを分けるため「状態保存用の列を2セット用意して管理する」アプローチを検討し、検証に約2週間を費やす。しかし後に「有給の失効ロジック」が完全に抜け落ちていることが発覚。
...
* **例外検知によるフェイルセーフ（`throw new Error`）**
  * 有給残高の不足など、データ不整合や運用上の致命的なエラーが発生した際は、曖昧に処理を進めず確実に処理を中断し、システムエラーとして即座に検知できる設計を採用。
* **パフォーマンスと負荷の最適化**
