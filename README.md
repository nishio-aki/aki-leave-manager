...
### ① 社員マスタ * **A列（社員ID）:** 一意の識別子（例: EMP001） * **B列（氏名）:** 社員名 * **C列（部署）:** 所属部署 * **D列（入社日）:** 自動付与の基準日判定に使用 * **E列（メールアドレス）:** 各種アラート・通知の送信先 ### ② 付与履歴 * **A列（付与ID）:** `社員ID_YYYYMMDD` で生成される一意のキー * **B列（社員ID）:** 紐づく社員 * **C列（付与日） / D列（有効期限）:** 有効期間の判定（FIFOの対象範囲） * **E列（付与日数） / F列（消費済日数）:** 残日数計算のソース * **G列・H列（通知フラグ）:** 6ヶ月前・1ヶ月前の通知済み管理 
### ③ 取得履歴（トランザクション） * **A列（タイムスタンプ）:** 申請日時 * **B列（社員ID）:** 申請者 * **C列（取得日） / D列（取得区分）:** 全休・半休の判定 * **E列（メールアドレス）:** 申請者メール * **F列（取得ID / `G-YYYYMMDD-HHmmss`）:** フォーム連携・一意性の担保 * **G列（ステータス）:** `承認待ち` / `承認済み` / `調整依頼中` / `【要】事務対応`
...
* **監視・ログ収集の高度化:** エラー発生時のSlack/Chatwork連携によるリアルタイム通知の拡充）。
